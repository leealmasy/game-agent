...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[previous_level]: 
            for effnode in a_node.effnodes:
                self.s_levels[level].add(effnode) 
                a_node.children.add(effnode), effnode.parents.add(a_node)  
        ...
        return self.s_levels.append(s_nodes)        
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node1= [effect for effect in node_a1.action.effect_add if effect in node_a2.action.effect_rem]
        node2= [effect for effect in node_a2.action.effect_add if effect in node_a1.action.effect_rem]
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        node1_add= [effect for effect in node_a1.action.effect_add if effect in node_a2.action.precond_neg]
        node1_remove= [effect for effect in node_a1.action.effect_rem if effect in node_a2.action.precond_pos]
        
        node2_add= [effect for effect in node_a2.action.effect_add if effect in node_a1.action.precond_neg]
        node2_remove= [effect for effect in node_a2.action.effect_rem if effect in node_a1.action.precond_pos]
        
        if node1_add or node1_remove or node2_add or node2_remove:
            return True
        else:
            return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_a1, precond_a2 in itertools.product(node_a1.parents, node_a2.parents):
           return precond_a1.is_mutex(precond_a2)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        cond_1= node_s1.symbol == node_s2.symbol 
        cond_2= node_s1.is_pos != node_s2.is_pos 
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        beenvisited = set()
        # if goal is reached add level -> level_sum
        for goal in self.problem.goal:
            goal_found = False
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if goal == state and (state not in beenvisited):
                        goal_found = True
                        level_sum += level
                        break
                    beenstated.add(state)
                if goal_found:
                    break
